...
for (let index = 0; index <= 14; index++) {
    const containerDiv = (container.innerHTML += `<div class="film-container">
        <div class="img-container">
        <i class="favorite fa-regular fa-star"></i>
        <i class="videoHD fa-solid fa-video"> H D </i>
        <img class="img-container" src="${movies[index].posterUrl}">
        </div>
        <div class="desc-container">
        <h4>${movies[index].title}</h4>
        <p>${movies[index].actors}</p>
        <span>${movies[index].year}</span>
        </div>`)
...
document.getElementById("slider").append(carousel);

for (let index = 0; index < movies.length; index++) {
    const carouselDiv = (carousel.innerHTML += `<div class="slide">
        <img class="img" src="${movies[index].posterUrl}">
        </div>
    `);
...
const favorite = document.querySelector(".favorite-container");
const favoriteBtn = document.querySelector(".favorite-button");

favoriteBtn.addEventListener("click", function () {
    favorite.classList.toggle("showFavorite")
});

const watchLater = document.querySelector(".watchLater-container");
const watchLaterBtn = document.querySelector(".watchLater-button");

watchLaterBtn.addEventListener("click", function () {
    watchLater.classList.toggle("show-watchLater")
});
